...
class RCA(object):
    ...
    def __init__(
        self, n_comp, upfact=1, ksig=3, n_scales=3, ksig_init=5, filters=None, verbose=2
    ):
        self.n_comp = n_comp
        self.upfact = upfact
        self.ksig = ksig
    ...
    def fit(
        ...
        self.im_hr_shape = (
            self.upfact * self.shap[0],
            self.upfact * self.shap[1],
            self.shap[2],
        )
        ...
        if obs_weights is None:
            ...
            self.obs_weights = (
                obs_weights
                / np.expand_dims(np.sum(obs_weights, axis=2), 2)
                * self.shap[2]
            )
        elif obs_weights.shape == (self.shap[2],):
            self.obs_weights = (
                obs_weights.reshape(1, 1, -1) / np.sum(obs_weights) * self.shap[2]
            )
        else:
            raise ValueError(
...
window of 7.5 pixels."""
    ...
    def _initialize(self):
        ...
        if self.shifts is None:
            ...
            for i in range(self.shap[2]):
                ...
                nsig_shifts = min(
                    self.ksig_init, 0.8 * self.obs_data[:, :, i].max() / self.sigs[i]
                )
                thresh_data[:, :, i] = utils.HardThresholding(
                    thresh_data[:, :, i], nsig_shifts * self.sigs[i]
                )
                cents += [
                    utils.CentroidEstimator(thresh_data[:, :, i], sig=self.psf_size)
                ]
    ...
    def _fit(self):
        ...
        if self.default_filters:
            self.Phi_filters = get_mr_filters(
                self.im_hr_shape[:2], opt=self.opt, coarse=True, trim=False
            )
        rho_phi = np.sqrt(np.sum(np.sum(np.abs(self.Phi_filters), axis=(1, 2)) ** 2))

        # Set up source updates, starting with the gradient
        source_grad = grads.SourceGrad(
            self.obs_data,
            self.obs_weights,
            weights,
            self.flux,
        ...
        sparsity_prox = rca_prox.StarletThreshold(
            0
        )  # we'll update to the actual thresholds later
        ...
        for k in range(self.nb_iter):
            ...
            tau = 1.0 / beta
            sigma = (1.0 / lin_recombine.norm**2) * beta / 2
    ...
    def estimate_psf(
        ...
        for j, pos in enumerate(test_pos):
            # determine neighbors
            nbs, pos_nbs = utils.return_neighbors(
                pos, self.obs_pos, self.A.T, n_neighbors
            )
            # train RBF and interpolate for each component
            for i in range(self.n_comp):
                rbfi = Rbf(
                    pos_nbs[:, 0], pos_nbs[:, 1], nbs[:, i], function=rbf_function
                )
        ...
        if apply_degradation:
            ...
            deg_PSFs = np.array(
                [
                    grads.degradation_op(
                        PSFs[:, :, j], shift_kernels[:, :, j], upfact, decim
                    )
...
def poly_pos(pos, max_degree=2, center_normalice=True, x_lims=None, y_lims=None):
