...
class CoeffGrad(GradParent, PowerMethod):
    """ Gradient class for the coefficient update. bla
    ...
    D: float
        Upsampling factor.
    """
    def __init__(self, data, weights, S, VT, flux, sig, ker, ker_rot, D,SNR_weights, data_type='float') :
    ...
    def update_S(self, new_S, update_spectral_radius=True):
        ...
        # normfacs = 1/self.sig # [TL] Possible BUG
    ...
    def cost(self, x, y=None, verbose=False):
        ...
        cost_val = 0.5 * np.linalg.norm(self.obs_weights * ((self._current_rec - self.obs_data)*self.SNR_weights)) ** 2
        return cost_val
    ...
    def get_grad(self, x):
        ...
        self.grad = self.MtX(self.obs_weights**2 * ((self.MX(x) - self.obs_data)*self.SNR_weights))
...
class SourceGrad(GradParent, PowerMethod):
    ...
    def cost(self, x, y=None, verbose=False):
        ...
        if isinstance(self._current_rec, type(None)):
            self._current_rec = self.MX(x)
        cost_val = 0.5 * np.linalg.norm((self._current_rec - self.obs_data)*self.SNR_weights) ** 2
        return cost_val
